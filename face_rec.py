...
# check if the added image contains only one face also rejects deepfakes
...
# save the image and also update log if user is unique username
def save_user(imgPath, username, flag=False):
...
# checks match for uploaded pic with saved users
def authenticateUserImage(imgPath, username):
...
# authenticateUserImage('unknown_faces/Gates(2).jfif', 'BillGates')
# authenticateUserImage('unknown_faces/gates_fake.jpg', 'BillGates')
